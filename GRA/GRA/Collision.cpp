...
namespace Collision
{
	...
	static sf::Uint8 getPixel(const TextureMask& mask, const sf::Texture& tex, uint32_t x, uint32_t y) {
		...
		long long dmp_check = x + y * tex.getSize().x;
	//	if (dmp_check < 0 || dmp_check >= mask.size()) return 0;				// ZMIANY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	public:
		...
		auto& get(const sf::Texture& tex, size_t hash) {
			auto pair = bitmasks.find(hash);
			if (pair == bitmasks.end())
			{

				return create(tex, tex.copyToImage(),hash);
			}

			return pair->second;
		}

		TextureMask& create(const sf::Texture& tex, const sf::Image& img, size_t hash) {
			TextureMask mask(tex.getSize().y * tex.getSize().x);
			for (uint32_t y = 0; y < tex.getSize().y; ++y) {
				for (uint32_t x = 0; x < tex.getSize().x; ++x) {
					mask[x + y * tex.getSize().x] = img.getPixel(x, y).a;
				}
			}
			bitmasks[hash] = std::move(mask);
			return bitmasks[hash];
		}
		...
		TextureMask& create(const sf::Texture& tex, const sf::Image& img) {
			...
			bitmasks[(size_t) &tex] = std::move(mask);
			return bitmasks[(size_t) & tex];
		}

	private:
		std::map<const size_t, TextureMask> bitmasks;
	};
	...
	bool singlePixelTest(const sf::Sprite& sprite, sf::Vector2f& mousePosition, sf::Uint8 alphaLimit) {
		...
		auto& mask = bitmasks().get(*sprite.getTexture(), typeid(sprite).hash_code());
	...
	bool pixelPerfectTest(const sf::Sprite& sprite1, const sf::Sprite& sprite2, sf::Uint8 alphaLimit) {
		...
		auto& mask1 = bitmasks().get(*sprite1.getTexture(), typeid (sprite1).hash_code());
		auto& mask2 = bitmasks().get(*sprite2.getTexture(), typeid (sprite2).hash_code());
